...
from ..models import EvList # importing Album, but this time going up one level into our albums folder using ..


class EVListSerializer(serializers.ModelSerializer):
    # define a Meta subclass that details which model and fields to serialize
    class Meta:
        model = EvList # define model to serialize from
