...
from .views import LivingResultsView, LivingEditView

urlpatterns = [
    path('', LivingResultsView.as_view()), # as_view passes the httprequest onto the request attribute on the view/controller
    path('edit/<int:pk>', LivingEditView.as_view()), # as_view passes the httprequest onto the request attribute on the view/controller
]