...
class PropertyWittleView(APIView):
    ...
    def get(self, _request):
        ...
        properties = Property.objects.prefetch_related(
          Prefetch('bars', queryset=PropertyBar.objects.filter(walking_time_mins__lte=4)),
          Prefetch('restaurants', queryset=PropertyRestaurant.objects.filter(walking_time_mins__lte=4)),
          Prefetch('primaries', queryset=PropertyPrimary.objects.filter(walking_time_mins__lte=4)),
          Prefetch('secondaries', queryset=PropertySecondary.objects.filter(walking_time_mins__lte=4)),
          Prefetch('gyms', queryset=PropertyGym.objects.filter(walking_time_mins__lte=4)),
          Prefetch('takeaways', queryset=PropertyTakeaways.objects.filter(walking_time_mins__lte=4)),
          Prefetch('tubes', queryset=PropertyTube.objects.filter(walking_time_mins__lte=4)),
          Prefetch('parks', queryset=PropertyPark.objects.filter(walking_time_mins__lte=4)),
          Prefetch('cafes', queryset=PropertyCafe.objects.filter(walking_time_mins__lte=4)),
          Prefetch('supermarkets', queryset=PropertySupermarket.objects.filter(walking_time_mins__lte=4)),
        )
