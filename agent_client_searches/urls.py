...
from .views import AddNewSearch, DeleteSearch, GetSearches, UpdateSearch

urlpatterns = [
    path('', GetSearches.as_view()), # as_view passes the httprequest onto the request attribute on the view/controller
    path('post/', AddNewSearch.as_view()), # as_view passes the httprequest onto the request attribute on the view/controller
