...
from ..models import PubList # importing Album, but this time going up one level into our albums folder using ..


class PubListSerializer(serializers.ModelSerializer):
    # define a Meta subclass that details which model and fields to serialize
    class Meta:
        model = PubList # define model to serialize from
