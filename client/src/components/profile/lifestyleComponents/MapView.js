...
const MapView = ({ masterLiving3, variableValue, setLifestyleView, detail1, detail2, detail3, startIndex, endIndex, handlePageClick,
  viewport, setViewport, lifestyleDropdown, dropdownValue, iconSetting, iconId, showPopup, lat }) => {
                                    ...
                                    {(showPopup & icon.id === iconId) && (
                                      <Popup key={index} id={icon.id} longitude={icon.long} latitude={icon[lat]} closeOnClick={false} className='icon-popup'>
                                        ...
                                        <div className='bottom-box'>
                                          ...
                                          {variableValue === 'restaurants' ? <h3>🍽 {icon[detail2]}</h3> : ''}
